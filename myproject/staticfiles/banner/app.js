const body = document.getElementById('bili_banner')
const lerp = (start, end, amt) => (1 - amt) * start + amt * end; // 计算线性插值
import { barnerImagesData1, barnerImagesData2 } from './banner_json.js'
...
function initItems() {
  ...
  if (layers.length <= 0) {
    ...
    body.style.display = ''
    layers = Array.from(document.querySelectorAll(".layer")); // Ensure layers is an array
  } else {
